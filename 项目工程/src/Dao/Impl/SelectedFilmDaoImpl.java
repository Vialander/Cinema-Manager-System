...
public class SelectedFilmDaoImpl extends BaseDao implements FilmDao {
    ...
    public int updateFilm(Film film){
        String sql = "update selectedfilm set mainactor = ?, score = ?, type = ?, timeLong = ?, country = ?, film_image = ?, start_time = ?, on_date = ?, down_date = ? where film_id = ? and film_name = ?";
    ...
    public int delFilm(Film film){
        String sql = "delete from selectedfilm where film_id = ? and film_name = ?";
        Object[]param = {film.getFilmId(),film.getFilmName()};
        return executeSql(sql,param);
    }

    public int addFilm(Film film){
        String sql = "insert into selectedfilm values(?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public ArrayList<Film> getAllFilm(){
        ...
        try{
            String sql = "select * from selectedfilm";
            conn = getConnection();
